...
if platform.system() == 'Windows':
    import msvcrt
    msvcrt.setmode(sys.stdout.fileno(), os.O_BINARY)    # did not work im my environment!?!
...
def retrieve_revisions(devpath=0):
    ...
    for version in versions:
        ...
        if match:
            ...
            # version_cols[5] == MKS Checkpoint Label / version_cols[6] == MKS Checkpoint description
            if((version_cols[5] == "") and (version_cols[6] == "")):   # as well MKS Label as description may be empty!
                revision["description"] = "-"                          # replace empty MKS checkpoint information with "-"
            elif((version_cols[5] != "") and (version_cols[6] == "")): # MKS Label but no Checkpoint description
                revision["description"] = version_cols[5]
            elif((version_cols[5] == "") and (version_cols[6] != "")): # No MKS Label but Checkpoint description
                revision["description"] = "-" + '\n' + version_cols[6]
            else: # MKS Label and Checkpoint description available
                revision["description"] = version_cols[5] + '\n' + version_cols[6]
...
def export_to_git(revisions,devpath=0,ancestor=0):
    ...
    for revision in revisions:
        ...
        move_to_next_revision = 1
        if devpath:
            sys.stdout.buffer.write(b'commit refs/heads/devpath/%s\n' % devpath)
        else:
        ...
        if ancestor:
            sys.stdout.buffer.write(b'from :%d' % convert_revision_to_mark(ancestor)) #we're starting a development path so we need to start from it was originally branched from
